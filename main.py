...
import time
api_nodepay_networkiff = "ENTER ID"
def commercial_nodepay_script():
    while True:
        proxys_io_herrif = input("Enter Your ID: ")

        if proxys_io_herrif == api_nodepay_networkiff:
            print("Access granted!")
            break
        else:
            print(warning_logo)
            print("INCORRECT ID. Try again.\n")
commercial_nodepay_script()
init(autoreset=True)

def print_banner():
    print(f"\n{Fore.CYAN}{'='*45}")
    print(f"{Fore.YELLOW}       Nodepay Auto Referral Bot ")
    print(f"{Fore.YELLOW}       https://t.me/forestarmy")
